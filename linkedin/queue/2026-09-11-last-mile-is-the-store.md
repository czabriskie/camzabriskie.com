...
One thing here really can't be taken back, and I had a second one wrong. I'd written in my own notes that losing the key you sign updates with meant the app could never be updated again. Not so, as long as you use Play App Signing, where Google holds the real signing key and you can request a reset of your upload key and be going again in a day or two. The unrecoverable version is opting out of that and then losing the signing key itself. The identifier that tells Android which app this is gets fixed the moment you publish, and that one genuinely can't be undone. I renamed mine beforehand, and because Android treats a changed identifier as a completely different app, the install started fresh with no save carried over and both icons on the home screen until I removed the old one. Fine before launch. After launch that same move wipes every player's progress, which isn't really a decision so much as a mistake you get to make once.

This is the shape of most software, not just phone games. Writing the code got dramatically cheaper, and that was already the fun part, while the distribution, the compliance, the two-week wait with actual humans in it, and the handful of choices that are permanent the first time you get them wrong all cost exactly what they did before.
