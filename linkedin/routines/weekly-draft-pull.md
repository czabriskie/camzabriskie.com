...
- **No URLs in the post body.** Links in the body suppress distribution. Name the source in prose (author, site, or title) and leave the URL out. The link goes into the post's first comment instead: write the exact first-comment text into the queue file's first_comment frontmatter field (one line). The publisher posts it as the first comment automatically right after the post goes live, and the daily nudge routine hands Cam the text to paste if that automatic comment fails. Carry the same text in the Signal Log entry footer.
- **The first comment is mandatory whenever there is something real to link (rule set 2026-08-31).** A post with a blog companion (opened this run or already published) links the blog post and only the blog post ('Wrote more about this on my site: <url>'). A post that references a source article with no blog companion links that article ('Here is a link to the full article: <url>'), using the URL from the Readwise highlight's metadata or one confirmed on the web; never an invented or unverified URL, and if the real URL genuinely cannot be confirmed, name the source in the body and leave first_comment out rather than guessing. Only a journal-sourced post with no article and no blog companion has no first_comment.
...
6. **Write the queue files and open ONE pull request for the batch.** In camzabriskie.com, branch `claude/linkedin-week-<YYYY-MM-DD>` (date from `date -I`). One file per draft in linkedin/queue/, named `YYYY-MM-DD-<short-slug>.md` after its suggested publish date, following linkedin/README.md exactly:
   ...
   - Frontmatter `first_comment`: the exact one-line comment text, required per the Distribution section's mandatory-first-comment rule: the blog URL when there is a companion (that link only), otherwise the referenced article's confirmed URL. Only link to a URL that exists or, for a companion blog post opened this run, the URL its slug will have. Omit the field only when the post genuinely has nothing to link (journal-sourced, no article, no companion).
