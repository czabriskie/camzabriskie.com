...
One thing a debug build gets you that a release build doesn't: `adb shell run-as com.rhythmgardens.app cat files/taps.log` reads a file straight out of the app's own private storage on the device, no root required, which is how this game's on-device play log actually gets pulled off a phone for a look.
